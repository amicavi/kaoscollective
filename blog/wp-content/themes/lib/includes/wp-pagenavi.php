...
function my_deregister_styles() {
	wp_deregister_style(  THEME_SLUG  );
}

function pagenavi_textdomain() {
	load_plugin_textdomain( THEME_SLUG , false,  THEME_SLUG );
}

function pagenavi_menu() {
	if (function_exists('add_theme_page')) {
		add_theme_page(__('PageNavi',  THEME_SLUG ), __('PageNavi',  THEME_SLUG ), 'manage_options', 'wp-pagenavi/pagenavi-options.php') ;
	}
}

function pagenavi_stylesheets() {
	if(@file_exists(TEMPLATEPATH.'/pagenavi-css.css')) {
		wp_enqueue_style( THEME_SLUG , get_stylesheet_directory_uri().'/pagenavi-css.css', false, '2.50', 'all');
	} else {
		wp_enqueue_style( THEME_SLUG , plugins_url('wp-pagenavi/pagenavi-css.css'), false, '2.50', 'all');
	}	
}
...
function pagenavi_init() {
	...
	$pagenavi_options['pages_text'] = __('Page %CURRENT_PAGE% of %TOTAL_PAGES%', THEME_SLUG );
	$pagenavi_options['current_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['page_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['first_text'] = __('&laquo; First', THEME_SLUG );
	$pagenavi_options['last_text'] = __('Last &raquo;', THEME_SLUG );
	$pagenavi_options['next_text'] = __('&raquo;', THEME_SLUG );
	$pagenavi_options['prev_text'] = __('&laquo;', THEME_SLUG );
	$pagenavi_options['dotright_text'] = __('...', THEME_SLUG );
	$pagenavi_options['dotleft_text'] = __('...', THEME_SLUG );
