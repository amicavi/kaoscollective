<?php
// Fist full of comments
if (!function_exists("tpo_comment")) {
	function tpo_comment($comment, $args, $depth) {
	   $GLOBALS["comment"] = $comment; ?>
	                 
		<li <?php comment_class(); ?>>
	    
	    	
	      	
	      	<div id="li-comment-<?php comment_ID() ?>" class="comment-container">
	      		<div class="comment-box" >
						<?php if(get_comment_type() == "comment"){ ?>
							<div class="avatar-box"><?php the_commenter_avatar($args) ?></div>
						<?php } ?>            
						<div class="comment-text">
						<div class="reply">
									<?php comment_reply_link(array_merge( $args, array("depth" => $depth, "max_depth" => $args["max_depth"]))) ?>
								</div><!-- reply end-->    
							<div class="comment-head">
										
								<span class="name"><?php the_commenter_link() ?></span>           
								<span class="date"><?php echo get_comment_date(get_option( "date_format" )) ?> <?php _e("at", THEME_SLUG); ?> <?php echo get_comment_time(get_option( "time_format" )); ?></span>
								<span class="perma"><a href="<?php echo get_comment_link(); ?>" title="<?php _e("Direct link to this comment", THEME_SLUG); ?>">#</a></span>
								<span class="edit"><?php edit_comment_link(__("Edit", THEME_SLUG), "", ""); ?></span>
												
							</div><!--  comment-head end -->
					  
							<div class="comment-entry"  id="comment-<?php comment_ID(); ?>">
							
							<span><?php echo $comment->comment_content; ?></span>
								
							<?php if ($comment->comment_approved == "0") { ?>
								<p class="unapproved"><?php _e("Your comment is awaiting moderation.", THEME_SLUG); ?></p>
							<?php } ?>
                        </div>    
                                           
                
                    </div><!-- comment-entry end  -->
				</div><!-- comment-text end  -->
			</div><!-- comment-container end -->
			
	<?php 
	}
}

// PINGBACK / TRACKBACK OUTPUT
if (!function_exists("list_pings")) {
	function list_pings($comment, $args, $depth) {
	
		$GLOBALS["comment"] = $comment; ?>
		
		<li id="comment-<?php comment_ID(); ?>">
			<span class="author"><?php comment_author_link(); ?></span> - 
			<span class="date"><?php echo get_comment_date(get_option( "date_format" )) ?></span>
			<span class="pingcontent"><?php comment_text() ?></span>
	
	<?php 
	} 
}
		
if (!function_exists("the_commenter_link")) {
	function the_commenter_link() {
	    $commenter = get_comment_author_link();
	    if ( preg_match( '/]* class=[^>]+>/', $commenter ) ) {$commenter = preg_replace( '/(]* class=[\'"]?)/', '\1url ' , $commenter );
	    } else { $commenter = preg_replace( '/(<a )/', '\\1class="url "' , $commenter );}
	    echo $commenter ;
	}
}

if (!function_exists("the_commenter_avatar")) {
	function the_commenter_avatar($args) {
	    $email = get_comment_author_email();
	    $avatar = str_replace( "class='avatar", "class='photo avatar", get_avatar( "$email",  $args['avatar_size']) );
	    echo $avatar;
	}
}

?>