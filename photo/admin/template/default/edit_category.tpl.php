...
<?php include_once(dirname(__FILE__) . '/category_related.tpl.php'); ?>
		...
		<h3><?php echo __('Édition de la catégorie') ?></h3>
...
<?php if ($tpl->disCategoryInfo('creatable_field')) : ?>
					<p class="field checkbox">
						<input<?php if (!$tpl->disCategoryInfo('creatable_parent')) : ?> disabled="disabled"<?php endif; ?><?php if ($tpl->disCategoryInfo('creatable')) : ?> checked="checked"<?php endif; ?> id="allow_create_<?php echo $tpl->getCategoryInfo('id'); ?>" name="<?php echo $tpl->getCategoryInfo('id'); ?>[creatable]" type="checkbox" />
						<span><label for="allow_create_<?php echo $tpl->getCategoryInfo('id'); ?>"><?php echo __('Autoriser la création de catégories'); ?></label></span>
					</p>
<?php endif; ?>
					<p class="field">
					...
					<p class="field">
						<label><?php echo __('Trier les catégories par :'); ?></label>
					</p>
...
<?php if ($tpl->disPerm('albums_modif')) : ?>
		...
		//<![CDATA[
		var confirm_delete = "<?php echo $tpl->getL10nJS(__('Êtes-vous sûr de vouloir supprimer cette catégorie, ainsi que toutes les images, commentaires, votes et tags qui s\'y trouvent ?')); ?>";
		//]]>
		</script>
		<form id="confirm_delete" class="form_page" action="" method="post">
			<div>
				...
				<input name="anticsrf" type="hidden" value="<?php echo $tpl->getAdmin('anticsrf'); ?>" />
				<input disabled="disabled" class="submit js_required" name="delete" type="submit" value="<?php echo __('Supprimer la catégorie'); ?>" />
			</div>
		</form>
<?php endif; ?>