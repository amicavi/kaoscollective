...
<?php include_once(dirname(__FILE__) . '/image_related.tpl.php'); ?>
		...
		<h3 class="h3_help_link">
			<span><?php echo __('Géolocalisation') ?></span>
<?php if ($tpl->disHelp()) : ?>
		...
		<form id="geoloc_form" class="form_page" action="" method="post">
			<div>
				<div id="geoloc_map">
					...
					//<![CDATA[
					var type = google.maps.MapTypeId.<?php echo $tpl->getAdmin('geoloc_type'); ?>;
					var latitude = '<?php echo $tpl->getImageInfo('latitude'); ?>';
					var longitude = '<?php echo $tpl->getImageInfo('longitude'); ?>';
					var unknown_address = "<?php echo $tpl->getL10nJS(__('Adresse inconnue.'), 70, "\n"); ?>";
					//]]>
					</script>
					<p class="field">
						<span<?php if ($tpl->getImageInfo('latitude') !== '' && $tpl->getImageInfo('longitude') !== '') : ?> style="display:none"<?php endif; ?> id="add_marker" class="icon icon_geomarker_add"><a href="javascript:;" class="js"><?php echo __('Ajouter le marqueur'); ?></a></span>
						<span<?php if ($tpl->getImageInfo('latitude') === '' || $tpl->getImageInfo('longitude') === '') : ?> style="display:none"<?php endif; ?> id="del_marker" class="icon icon_geomarker_del"><a href="javascript:;" class="js"><?php echo __('Supprimer le marqueur'); ?></a></span>
					</p>
				</div>
				<div id="geoloc_coords">
					<div id="geoloc_coords_tools">
						<p class="field field_ftw">
							...
							&nbsp;
							<select id="places">
								<?php echo $tpl->getImageInfo('places'); ?>
					...
					<p class="field field_ftw">
						...
						<input name="<?php echo $tpl->getImageInfo('id'); ?>[latitude]" size="40" maxlength="25" id="latitude" type="text" class="text" value="<?php echo $tpl->getImageInfo('latitude'); ?>" />
					</p>
					<p class="field field_ftw">
						<label for="longitude"><?php echo __('Longitude :'); ?></label>
						<input name="<?php echo $tpl->getImageInfo('id'); ?>[longitude]" size="40" maxlength="25" id="longitude" type="text" class="text" value="<?php echo $tpl->getImageInfo('longitude'); ?>" />
					</p>
					<p class="field field_ftw">
						<label for="place"><?php echo __('Lieu :'); ?></label>
						<input name="<?php echo $tpl->getImageInfo('id'); ?>[place]" size="40" maxlength="100" id="place" type="text" class="text" value="<?php echo $tpl->getImageInfo('place'); ?>" />
					</p>
				</div>
				<div class="clear"></div>
