...
<?php if ($tpl->disPerm('users_groups') && $tpl->disCategoryInfo('password_groups')) : ?>
					<p><?php echo __('Groupes ayant accès à cet album :'); ?></p>
					<ul>
						<?php echo $tpl->getCategoryInfo('password_groups'); ?>
...
<?php endif; ?>
		...
		<div class="related">
			...
			<select onchange="window.location.href='<?php echo $tpl->getAdmin('admin_base_url'); ?>'+this.options[this.selectedIndex].value">
				<optgroup label="<?php echo __('Album'); ?>">
<?php if ($tpl->disPerm('albums_modif') && !$tpl->disCategoryInfo('empty')) : ?>
					<option value="thumb-album/<?php echo $_GET['object_id']; ?>"<?php if ($_GET['section'] == 'thumb-album') : ?> class="selected" selected="selected"<?php endif; ?>><?php echo __('modifier la vignette'); ?></option>
...
<?php endif; ?>
					<option value="edit-album/<?php echo $_GET['object_id']; ?>"<?php if ($_GET['section'] == 'edit-album') : ?> class="selected" selected="selected"<?php endif; ?>><?php echo __('éditer'); ?></option>
<?php if ($tpl->disPerm('albums_edit')) : ?>
					<option value="geoloc-album/<?php echo $_GET['object_id']; ?>"<?php if ($_GET['section'] == 'geoloc-album') : ?> class="selected" selected="selected"<?php endif; ?>><?php echo __('géolocaliser'); ?></option>
<?php endif; ?>
<?php if ($tpl->disPerm('albums_modif')) : ?>
					<option value="watermark-album/<?php echo $_GET['object_id']; ?>"<?php if ($_GET['section'] == 'watermark-album') : ?> class="selected" selected="selected"<?php endif; ?>><?php echo __('modifier le filigrane'); ?></option>
<?php endif; ?>
				</optgroup>
				<optgroup label="<?php echo $tpl->getSectionsList('objects_text'); ?>">
					<option value="album/<?php echo $tpl->getSectionsList('filter_url'); ?>"<?php if ($_GET['section'] == 'album') : ?> class="selected" selected="selected"<?php endif; ?>><?php echo __('gérer les images'); ?></option>
<?php if ($tpl->disPerm('albums_edit') && !$tpl->disCategoryInfo('empty')) : ?>
					<option value="mass-edit-album/<?php echo $tpl->getSectionsList('filter_url'); ?>"<?php if ($_GET['section'] == 'mass-edit-album') : ?> class="selected" selected="selected"<?php endif; ?>><?php echo __('édition en masse'); ?></option>
<?php endif; ?>
<?php if (!$tpl->disCategoryInfo('empty') && $tpl->disPerm('albums_modif') && $tpl->disPosition('normal')) : ?>
					<option value="sort-album/<?php echo $_GET['object_id']; ?>"<?php if ($_GET['section'] == 'sort-album') : ?> class="selected" selected="selected"<?php endif; ?>><?php echo __('trier les images'); ?></option>
<?php endif; ?>
				</optgroup>
			</select>
		</div>